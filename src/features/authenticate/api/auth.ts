 ...
 */
export function refreshTokenApi(payload: RefreshTokenRequest) {
...
export async function initGoogleOAuthApi(params: {
  ...
  if (!data.success)
    throw new Error(data.message ?? 'Init Google OAuth failed');
  return data;
}

export async function googleCallbackApi(params: {
  code: string;
  state: string;
}) {
  const url = new URL(
