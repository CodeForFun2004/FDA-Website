...
function WaterTooltip({ active, label, payload }: any) {
  ...
  return (
    <div className="rounded-xl border bg-background px-4 py-3 shadow-md">
      ...
      <div className="text-sm text-primary">value : {payload[0].value}</div>
    </div>
  );
}
...
export function DashboardView({ onCreateAlert, onAddDevice }: DashboardViewProps) {
  const { totalDevices, offlineDevices, activeAlerts, monitoredZones } = useDashboardStats();
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Real-time flood monitoring summary</p>
        </div>

        <div className="flex gap-3">
          <Button variant="outline" className="rounded-full" onClick={onCreateAlert}>
            <Plus className="mr-2 h-4 w-4" /> Create Alert
          </Button>
          <Button className="rounded-full shadow-lg shadow-blue-500/20" onClick={onAddDevice}>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4 shadow-sm border-none bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">River Sensor A1 - Last 24 Hours</p>
          </CardHeader>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-sm mt-2 text-emerald-600 font-medium">Safe (Below 4.0m)</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-sm mt-2 text-muted-foreground">2 Blocked Routes</p>
          </CardContent>
        </Card>
      </div>
    </div>
